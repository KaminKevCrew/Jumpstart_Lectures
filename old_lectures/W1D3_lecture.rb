...
# loop through some amount of numbers (i) that are smaller than the one passed in

# if number % i == 0 => false

# at the end if we haven't found any factors, then our number is prime.

def is_prime?(number)
  return false if number < 2
  return true if number == 2
  return false unless number % 1 == 0
  i = 2
  while i < number
    if number % i == 0
      return false
